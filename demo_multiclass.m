...
for t = 1:T
    ...
    Ytrain{t} = ??; %should be an Ntra x L matrix containing class labels of task t (contains only -1s and +1s) where L is the number of classes
end

%perform training
state = kbtl_supervised_multilabel_classification_variational_train(Ktrain, Ytrain, parameters);
...
%perform prediction
prediction = kbtl_supervised_multilabel_classification_variational_test(Ktest, state);
